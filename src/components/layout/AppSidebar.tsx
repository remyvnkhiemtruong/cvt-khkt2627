...
import { AcademicCapIcon, ArrowsRightLeftIcon, BookOpenIcon, ChartBarIcon, ChevronLeftIcon, ChevronRightIcon, ClipboardDocumentCheckIcon, DocumentTextIcon, FolderIcon, HomeIcon, PlusCircleIcon, ShieldCheckIcon, SparklesIcon, UserGroupIcon } from '@heroicons/react/24/outline';
...
export const AppSidebar:React.FC<AppSidebarProps>=({currentView,onNavigate,isCollapsed=false,onToggleCollapse,className})=>{
  ...
  const items:NavItem[]=user.role==='student'?[{id:'dashboard',label:'Tổng quan',icon:HomeIcon},{id:'assignment-list',label:'Nhiệm vụ',icon:BookOpenIcon},{id:'portfolio-list',label:'Hồ sơ đọc',icon:FolderIcon},{id:'editor',label:'Bài đang viết',icon:DocumentTextIcon,params:{assignmentId:'assign-vo-nhat'}},{id:'version-diff',label:'So sánh phiên bản',icon:ArrowsRightLeftIcon,params:{assignmentId:'assign-vo-nhat'}},{id:'student-analytics',label:'Tiến bộ',icon:ChartBarIcon}]:
  user.role==='teacher'?[{id:'teacher-dashboard',label:'Tổng quan giáo viên',icon:HomeIcon},{id:'class-analytics',label:'Lớp & Heatmap',icon:UserGroupIcon},{id:'portfolio-list',label:'Hồ sơ học sinh',icon:FolderIcon},{id:'teacher-review',label:'Chấm & nhận xét',icon:ClipboardDocumentCheckIcon},{id:'ai-workspace',label:'Duyệt phản hồi AI',icon:SparklesIcon},{id:'assignment-builder',label:'Tạo nhiệm vụ',icon:PlusCircleIcon},{id:'rubric-management',label:'Rubric',icon:AcademicCapIcon},{id:'literature-texts',label:'Kho tác phẩm',icon:BookOpenIcon}]:
  user.role==='admin'?[{id:'admin-view',label:'Quản trị',icon:ShieldCheckIcon},{id:'teacher-dashboard',label:'Dữ liệu giảng dạy',icon:HomeIcon},{id:'ai-workspace',label:'AI Review Queue',icon:SparklesIcon},{id:'class-analytics',label:'Analytics',icon:ChartBarIcon},{id:'researcher-view',label:'Nghiên cứu',icon:AcademicCapIcon}]:
  user.role==='researcher'?[{id:'researcher-view',label:'Nghiên cứu',icon:AcademicCapIcon},{id:'portfolio-list',label:'Hồ sơ',icon:FolderIcon},{id:'class-analytics',label:'Analytics',icon:ChartBarIcon},{id:'version-diff',label:'Tiến trình',icon:ArrowsRightLeftIcon,params:{assignmentId:'assign-vo-nhat'}}]:
  ...
  return <aside className={cn('hidden min-h-[calc(100vh-3.5rem)] shrink-0 flex-col justify-between border-r border-slate-200 bg-white p-3 transition-all md:flex',isCollapsed?'w-16':'w-64',className)}>
    ...
    {!isCollapsed&&<div className="rounded-xl border bg-slate-50 p-3 text-[10px] leading-5 text-slate-500"><b className="block text-xs text-slate-800">Production V2</b>Dữ liệu học tập từ PostgreSQL • Version bất biến • AI review có giáo viên duyệt.</div>}
  </aside>;
};
