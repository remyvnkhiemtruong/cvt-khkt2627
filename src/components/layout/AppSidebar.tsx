...
export const AppSidebar:React.FC<AppSidebarProps>=({currentView,onNavigate,isCollapsed=false,onToggleCollapse,className})=>{
  ...
  user.role==='admin'?
    [{id:'admin-view',label:'Quản trị',icon:ShieldCheckIcon},{id:'teacher-dashboard',label:'Dữ liệu giảng dạy',icon:HomeIcon},{id:'ai-workspace',label:'Hàng đợi phản hồi AI',icon:SparklesIcon},{id:'class-analytics',label:'Phân tích lớp',icon:ChartBarIcon},{id:'researcher-view',label:'Nghiên cứu',icon:AcademicCapIcon}]:
  user.role==='researcher'?
    [{id:'researcher-view',label:'Nghiên cứu',icon:AcademicCapIcon},{id:'portfolio-list',label:'Hồ sơ',icon:FolderIcon},{id:'class-analytics',label:'Phân tích lớp',icon:ChartBarIcon}]:
  user.role==='ai'?[{id:'ai-workspace',label:'Hàng đợi phản hồi AI',icon:SparklesIcon}]:
  [{id:'portfolio-list',label:'Hồ sơ phản biện',icon:FolderIcon},{id:'teacher-review',label:'Đánh giá bạn học',icon:ClipboardDocumentCheckIcon}];

  return <aside className={cn('hidden min-h-[calc(100vh-3.5rem)] shrink-0 flex-col justify-between border-r border-slate-200 bg-white p-3 transition-all md:flex',isCollapsed?'w-16':'w-60',className)}>
    <div>
      <div className="mb-3 flex items-center justify-between border-b border-slate-100 px-2 pb-2">
        {!isCollapsed && <span className="text-xs font-semibold text-slate-500">{ROLE_NAMES[user.role] || user.role}</span>}
        {onToggleCollapse && (
          <button onClick={onToggleCollapse} className="mx-auto rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600">
      ...
      <nav className="space-y-0.5">
        {items.map(item => {
          const active = currentView === item.id, Icon = item.icon;
          const button = (
            <button
              type="button"
          ...
          return isCollapsed ? <Tooltip key={item.id} content={item.label} position="right">{button}</Tooltip> : <React.Fragment key={item.id}>{button}</React.Fragment>;
        })}
      </nav>
    </div>
    {!isCollapsed && (
      <div className="border-t border-slate-100 px-2 pt-2 text-[11px] text-slate-400">
        Học tốt Ngữ Văn
      </div>
    )}
  </aside>;
