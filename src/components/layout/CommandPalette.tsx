...
export const CommandPalette:React.FC<CommandPaletteProps>=({isOpen,onClose,onNavigate})=>{
  ...
  return <div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-900/50 px-4 pt-20 backdrop-blur-xs">
    <div className="flex w-full max-w-xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-modal">
      ...
      <div className="max-h-80 space-y-1 overflow-y-auto p-2">{filteredCommands.length===0?<div className="py-8 text-center text-xs text-slate-400">Không tìm thấy kết quả phù hợp với “{query}”.</div>:filteredCommands.map((command,index)=>{const Icon=command.icon,isSelected=index===selectedIndex;return <button type="button" key={command.id} onClick={()=>select(command)} onMouseEnter={()=>setSelectedIndex(index)} className={`flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-xs transition-colors ${isSelected?'bg-indigo-50 font-semibold text-indigo-950':'text-slate-700 hover:bg-slate-50'}`}><div className="flex min-w-0 items-center gap-3"><div className={`shrink-0 rounded p-1.5 ${isSelected?'bg-indigo-600 text-white':'bg-slate-100 text-slate-500'}`}><Icon className="h-4 w-4"/></div><div className="min-w-0"><div className="truncate">{command.title}</div>{command.subtitle&&<p className="truncate text-[11px] font-normal text-slate-400">{command.subtitle}</p>}</div></div><span className="ml-2 shrink-0 text-[10px] text-slate-400">{command.category}</span></button>;})}</div>
