...
export const MainLayout: React.FC<MainLayoutProps> = ({
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 font-sans text-slate-900">
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:left-2 focus:top-2 focus:z-50 focus:rounded-lg focus:bg-indigo-900 focus:px-4 focus:py-2 focus:font-bold focus:text-white focus:shadow-lg focus:outline-none focus:ring-2 focus:ring-white"
      >
      ...
      {currentUser.role === 'student' && (
        <StudentBottomNav currentView={currentView} onNavigate={onNavigate} />
      )}
      ...
      <footer className="hidden border-t border-slate-200 bg-white py-4 text-center text-xs text-slate-500 md:block">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 px-4 sm:flex-row">
          <span>
            <strong>Hồ Sơ Đọc Số THPT</strong> • Nền tảng phát triển năng lực đọc hiểu theo trục thi pháp
          </span>
