...
export const StudentBottomNav: React.FC<StudentBottomNavProps> = ({ currentView, onNavigate }) => {
  ...
  return (
    <nav aria-label="Thanh điều hướng di động" className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-around border-t border-slate-200 bg-white px-2 py-1 shadow-lg md:hidden">
      {items.map((item) => {
