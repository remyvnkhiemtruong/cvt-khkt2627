...
export const ToastItem: React.FC<ToastItemProps> = ({ toast, onDismiss }) => {
  ...
  return (
    <div
      className={cn(
        "flex items-start gap-3 p-3.5 bg-white border border-slate-200 rounded-xl shadow-elevated text-xs text-slate-800 animate-fade-in min-w-[280px] max-w-sm pointer-events-auto"
      )}
    >
      {icons[toast.type]}
