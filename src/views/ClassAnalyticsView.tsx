...
export const ClassAnalyticsView: React.FC<ClassAnalyticsViewProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="max-w-7xl space-y-6 pb-16">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 rounded-md border border-slate-200 bg-slate-50 p-3.5 text-sm text-slate-700"><span><strong>{uniqueStudents}</strong> học sinh</span><span>·</span><span><strong>{submittedStudents}</strong> học sinh đã nộp V1+</span><span>·</span><span>Điểm trung bình: <strong>{typeof overall === 'number' ? `${overall.toFixed(1)}%` : '—'}</strong> ({perStudentOverall.length} học sinh đã chấm)</span>{weakAxis && <><span>·</span><span>Cần lưu ý: <strong>{axisLabels[weakAxis]}</strong> ({Number(axisStats[weakAxis].average).toFixed(1)}%, n={axisStats[weakAxis].n})</span></>}</div>
