...
export const LiteratureTextsView: React.FC<LiteratureTextsViewProps> = ({ onNavigate }) => {
  ...
  const [selected, setSelected] = useState<(typeof literatureTexts)[number]|null>(null);
  ...
  const filtered = useMemo(
    () => literatureTexts.filter(t => `${t.title} ${t.author}`.toLowerCase().includes(search.toLowerCase().trim())),
  ...
  return (
    <div className="max-w-6xl space-y-6 pb-16">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-baseline sm:justify-between border-b border-slate-200 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Danh mục tác phẩm và ngữ liệu văn học dùng cho các nhiệm vụ đọc hiểu
          </p>
        </div>
      ...
      <FilterBar
        ...
        searchPlaceholder="Tìm kiếm theo tác phẩm hoặc tác giả..."
        onResetFilters={() => setSearch('')}
      />

      {/* Table of Literature Texts */}
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-slate-200 bg-slate-50/70 text-xs font-medium text-slate-600">
              <tr>
                ...
                <th className="py-3 px-4">Năm / Giai đoạn</th>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-xs text-slate-500">
                    {isLoading ? 'Đang tải...' : 'Không tìm thấy tác phẩm phù hợp.'}
                  </td>
                </tr>
              ) : (
                filtered.map(t => (
      ...
      <Modal
        ...
        footer={
          <div className="flex items-center justify-end gap-2">
            ...
            <Button variant="primary" size="sm" onClick={save} disabled={!title.trim() || !author.trim()}>
              Lưu tác phẩm
            </Button>
          </div>
        }
      >
        <div className="grid gap-3 sm:grid-cols-2 text-sm">
          ...
          <div className="sm:col-span-2">
            <label className="block text-xs font-medium text-slate-700 mb-1">Tóm tắt tác phẩm</label>
            <textarea
              className="w-full rounded-md border border-slate-300 p-2 text-sm outline-none focus:border-slate-500"
              rows={2}
              value={synopsis}
          ...
          <div className="sm:col-span-2">
            <label className="block text-xs font-medium text-slate-700 mb-1">Đoạn trích dẫn</label>
            <textarea
              className="w-full rounded-md border border-slate-300 p-2 text-sm outline-none focus:border-slate-500 font-sans"
              rows={4}
              value={excerpt}
          ...
          <div className="sm:col-span-2">
            <label className="block text-xs font-medium text-slate-700 mb-1">Bối cảnh sáng tác</label>
            <textarea
              className="w-full rounded-md border border-slate-300 p-2 text-sm outline-none focus:border-slate-500"
              rows={2}
              value={context}
        ...
        {selected && (
          <div className="space-y-4 text-sm text-slate-700">
            <div className="text-xs text-slate-500">
              Tác giả: <strong className="text-slate-900">{selected.author}</strong> · Thể loại: {selected.genre || 'Văn học'} · Năm: {selected.year || '—'}
            </div>

            {selected.synopsis && (
              <div className="space-y-1">
                <span className="font-medium text-slate-900 text-xs block">Tóm tắt:</span>
            ...
            {selected.excerpt && (
              <div className="space-y-1">
                <span className="font-medium text-slate-900 text-xs block">Đoạn trích tác phẩm:</span>
                <div className="whitespace-pre-wrap text-sm leading-7 text-slate-800 bg-white p-3 rounded-md border border-slate-200 max-h-64 overflow-y-auto">
                  {selected.excerpt}
                </div>
              </div>
            )}
            ...
            {selected.historicalContext && (
              <div className="space-y-1">
                <span className="font-medium text-slate-900 text-xs block">Bối cảnh sáng tác:</span>
