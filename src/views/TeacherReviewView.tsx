...
export const TeacherReviewView: React.FC<TeacherReviewViewProps> = ({ studentId = '', assignmentId = '', isPeerMode = false, onNavigate }) => {
  ...
  if(isLoading&&queue.length===0)return <StatePanel loading title="Đang tải bài nộp" message="Hệ thống đang lấy danh sách hồ sơ nộp bài."/>;
  ...
  const createFeedback=()=>{
    ...
    if(!selectedVersion){addToast({type:'warning',title:'Học sinh chưa nộp phiên bản',message:'Chỉ phản hồi sau khi học sinh đã tạo bản lưu.'});return;}
    addAnchoredFeedback({
      ...
      <header className="border-b border-slate-200 bg-white p-3 sm:px-5 sm:py-3">
        <div className="mx-auto flex max-w-7xl flex-col justify-between gap-3 lg:flex-row lg:items-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-medium text-slate-600">{currentIndex+1}/{queue.length}</span>
        ...
        <aside className="space-y-3">
          <section className="rounded-lg border border-slate-200 bg-white p-3.5">
            ...
            <div className="mt-3">
              ...
              <select value={selectedVersion} onChange={e=>setSelectedVersion(e.target.value)} className="mt-1 w-full rounded-md border border-slate-300 px-2.5 py-1.5 text-xs font-medium outline-none focus:border-indigo-500">
                <option value="">Bản nháp hiện tại</option>
                {currentPortfolio.versions.map(version=><option key={version.id} value={version.versionNumber}>{version.versionNumber}</option>)}
              </select>
            </div>
          </section>
            ...
            <div className="max-h-[55vh] space-y-1 overflow-y-auto">
              {queue.map((item,index)=>(
                <button
                  ...
                  className={`w-full rounded-md px-2.5 py-2 text-left text-xs transition-colors ${index===currentIndex?'bg-slate-100 text-slate-900 font-semibold':'text-slate-700 hover:bg-slate-50'}`}
                >
        ...
        <main className="space-y-4">
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-4">
            <div className="mb-3 flex flex-wrap gap-1.5">
              {POETIC_AXES.map(axis=>(
                <button
                  ...
                  className={`rounded-md px-2.5 py-1.5 text-xs font-medium transition-colors ${activeAxisId===axis.id?'bg-slate-900 text-white':'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                >
                  {axis.shortName}
                </button>
              ))}
            ...
            <div className="border-t border-slate-100 pt-3">
              <div className="mb-2 flex items-center justify-between gap-2">
                ...
                <Badge variant={selectedVersion?'blue':'slate'} size="sm">{selectedVersion||'Bản nháp'}</Badge>
              </div>
              <div onMouseUp={captureSelection} className="min-h-64 whitespace-pre-wrap rounded-md border border-slate-200 bg-slate-50/70 p-4 font-sans text-xs leading-6 text-slate-800">
                {activeResponse?.analysisText?.trim() || <span className="italic text-slate-400">Học sinh chưa viết nội dung ở trục này.</span>}
              </div>
              {activeResponse?.evidenceQuotes?.length>0&&(
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xs font-semibold text-slate-900">Phản hồi đã có</h2>
            </div>
            {currentFeedbacks.length===0 ? (
              <p className="mt-2 text-xs text-slate-500">Chưa có phản hồi ở trục này.</p>
            ) : (
                ...
                {currentFeedbacks.map(item=>(
                  <div key={item.id} className="rounded-md border border-slate-200 bg-slate-50 p-2.5 text-xs">
                    <div className="flex items-center justify-between">
                      <strong className="text-slate-800">{item.authorName}</strong>
        ...
        <aside className="space-y-4">
          <section className="rounded-lg border border-slate-200 bg-white p-4">
            <h2 className="flex items-center gap-2 text-xs font-semibold text-slate-900">
              <ChatBubbleLeftRightIcon className="h-4 w-4 text-slate-600"/>Gửi phản hồi
            </h2>
            {selectedText&&(
            ...
            />
            <Button className="mt-2 w-full" variant="primary" size="sm" disabled={!selectedVersion} onClick={createFeedback}>
              Gửi phản hồi
            </Button>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-4">
                    ...
                    <div className="grid grid-cols-4 gap-1">
                      {[1,2,3,4].map(level=>(
                        <button
                          ...
                          className={`rounded-md border py-1 text-xs font-semibold ${rubricScores[criterion.id]===level?'border-emerald-700 bg-emerald-700 text-white':'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
                        >
                          {level}
                        </button>
                      ))}
            ...
            <div className="mt-2.5 flex items-center justify-between text-xs">
              ...
              <strong className="text-sm font-semibold text-slate-900">{totalScore}/{maxScore}</strong>
            </div>
            <Button
