...
export const TeacherReviewView: React.FC<TeacherReviewViewProps> = ({
  ...
  if (isLoading && queue.length === 0) return <StatePanel loading title="Đang tải bài" message="Đang tải các bài đã nộp..." />;
        ...
        <main className="min-w-0 space-y-5 p-4 sm:p-6 lg:p-7">
          ...
          {pendingAiProposal && evaluatorRole === 'teacher' && (
            <section className="space-y-3 rounded-xl border border-sky-200 bg-sky-50/70 p-4 shadow-xs">
              <div className="flex flex-wrap items-center justify-between gap-2"><div><div className="text-xs font-bold uppercase tracking-wider text-sky-700">Góp ý AI</div><h3 className="mt-0.5 text-sm font-bold text-slate-900">Góp ý AI đã gửi học sinh</h3></div><Badge variant="blue">Chờ giáo viên xem</Badge></div>
              ...
              <p className="text-xs leading-5 text-slate-500">Học sinh đã thấy góp ý này. Giáo viên có thể thêm nhận xét riêng.</p>
          ...
          <section className="space-y-4">
            <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-slate-200 pb-3"><div><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Bản đã nộp</div><h2 className="mt-1 text-lg font-bold tracking-tight text-slate-950">{POETIC_AXES.find(axis => axis.id === activeAxisId)?.title}</h2></div><div className="flex items-center gap-2"><Badge variant="outline">{selectedSnapshot.versionNumber}</Badge><span className="text-xs text-slate-400">{activeResponse?.analysisText ? `${activeResponse.analysisText.trim().split(/\s+/).length} từ` : '0 từ'}</span></div></div>
