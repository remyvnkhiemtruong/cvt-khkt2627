  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      for (const [criterionId, value] of Object.entries(existing.criterionScores || {})) {
        loaded[criterionId] = Number(value.score || value.level || 0);
      }
      setRubricScores(loaded);
      setOverallFeedback(existing.overallFeedback || '');
    } else {
      setRubricScores(Object.fromEntries((activeRubric.criteria || []).map(criterion => [criterion.id, 0])));
      setOverallFeedback('');
    }
    ...
    try {
      ...
      const response = await fetch('/api/academic/action', {
        ...
        body: JSON.stringify({
          ...
          finalResponse,
          axisId: activeAxisId,
          teacherNote: decision === 'revised'
            ? 'Giáo viên chỉnh sửa đề xuất AI'
            : decision === 'rejected'
              ? 'Giáo viên không sử dụng đề xuất AI'
              : 'Giáo viên duyệt đề xuất AI'
        })
      });
      ...
      addToast({
        ...
        message: decision === 'rejected'
          ? 'Hồ sơ đang chờ nhận xét riêng của giáo viên.'
          : 'Phản hồi chính thức đã được lưu với vai trò giáo viên.'
      });
    } catch (error: unknown) {
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white px-4 py-2.5">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="text-sm"><strong className="text-slate-900">{currentPortfolio.studentName}</strong><span className="text-slate-500"> · {currentPortfolio.className || '—'} · {assignment.title}</span></div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-slate-500">{currentIndex + 1}/{queue.length}</span>
            <Button size="sm" variant="outline" disabled={currentIndex >= queue.length - 1} onClick={() => changeStudent(1)}>Sau <ChevronRightIcon className="ml-1 h-4 w-4" /></Button>
          </div>
        </div>
      </header>

      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 lg:grid-cols-[220px_minmax(0,1fr)_340px]">
        <aside className="space-y-3 border-b border-slate-200 bg-slate-50/50 p-3 lg:border-b-0 lg:border-r">
          <label className="block text-sm font-medium text-slate-600">Phiên bản chấm</label>
          <select value={selectedVersion} onChange={event => setSelectedVersion(event.target.value)} className="w-full rounded-md border border-slate-300 bg-white px-2 py-1.5 text-sm outline-none focus:border-slate-500">
        ...
        <main className="space-y-5 p-4 sm:p-6">
          <div className="flex gap-1 overflow-x-auto border-b border-slate-200">
            {POETIC_AXES.map(axis => <button key={axis.id} type="button" onClick={() => setActiveAxisId(axis.id)} className={`-mb-px whitespace-nowrap border-b-2 px-3 py-2 text-sm ${activeAxisId === axis.id ? 'border-slate-900 font-medium text-slate-900' : 'border-transparent text-slate-500'}`}>{axis.shortName}</button>)}
          </div>

          {pendingAiProposal && evaluatorRole === 'teacher' && <section className="space-y-3 rounded-md border border-slate-300 bg-slate-50 p-3">
            <div className="flex items-center justify-between"><h3 className="text-sm font-semibold text-slate-800">Đề xuất AI</h3><span className="text-sm text-slate-500">Chờ duyệt</span></div>
            {isEditingAiProposal ? <textarea rows={4} value={editedAiText} onChange={event => setEditedAiText(event.target.value)} className="w-full rounded-md border border-slate-300 bg-white p-2 text-sm" /> : <p className="whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{pendingAiProposal.response}</p>}
            <div className="flex flex-wrap gap-2">
              <Button size="sm" variant="primary" isLoading={isSubmittingAiReview} onClick={() => handleTeacherAiDecision(isEditingAiProposal ? 'revised' : 'approved')}>{isEditingAiProposal ? 'Sửa và gửi' : 'Duyệt và gửi'}</Button>
              {!isEditingAiProposal && <Button size="sm" variant="outline" onClick={() => setIsEditingAiProposal(true)}>Chỉnh sửa</Button>}
              ...
              <Button size="sm" variant="ghost" isLoading={isSubmittingAiReview} onClick={() => handleTeacherAiDecision('rejected')}>Không dùng AI</Button>
            </div>
          </section>}

          <section className="space-y-3">
            <div className="flex items-baseline justify-between border-b border-slate-100 pb-2"><h2 className="text-base font-medium text-slate-900">{POETIC_AXES.find(axis => axis.id === activeAxisId)?.title}</h2><span className="text-sm text-slate-500">{selectedSnapshot.versionNumber}</span></div>
            <div onMouseUp={captureSelection} className="min-h-56 select-text whitespace-pre-wrap text-sm leading-7 text-slate-800">{activeResponse?.analysisText?.trim() || <span className="italic text-slate-400">Không có nội dung ở trục này.</span>}</div>
            {activeResponse?.evidenceQuotes?.length ? <div className="space-y-1 border-t border-slate-100 pt-3"><div className="text-sm font-medium text-slate-600">Dẫn chứng</div>{activeResponse.evidenceQuotes.map(item => <blockquote key={item.id} className="border-l-2 border-slate-300 pl-3 text-sm italic text-slate-600">{item.text}</blockquote>)}</div> : null}
          </section>
        ...
        <aside className="space-y-6 border-t border-slate-200 bg-slate-50/40 p-4 lg:border-l lg:border-t-0">
          <section className="space-y-2">
            <h2 className="text-sm font-semibold text-slate-800">Nhận xét trực tiếp</h2>
            {selectedText && <div className="rounded border-l-2 border-slate-400 bg-white p-2 text-sm italic text-slate-600">“{selectedText}”</div>}
            <textarea rows={4} value={feedbackText} onChange={event => setFeedbackText(event.target.value)} placeholder="Nhập nhận xét cụ thể..." className="w-full rounded-md border border-slate-300 bg-white p-2 text-sm" />
          ...
          {literatureText && <div className="border-t border-slate-200 pt-3 text-sm text-slate-500">Ngữ liệu: {literatureText.title} · {literatureText.author}</div>}
        </aside>
      </div>
    </div>
  );
