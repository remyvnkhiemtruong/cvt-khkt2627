...
export const TeacherReviewView: React.FC<TeacherReviewViewProps> = ({
  ...
  const saveRubric = async () => {
    ...
    try {
      ...
      addToast({ type: 'success', title: 'Đã lưu điểm rubric', message: `Đã lưu điểm ${canonicalTotal}/${canonicalMax}.` });
    } catch (error: unknown) {
  ...
  const handleTeacherAiDecision = async (decision: 'approved' | 'revised' | 'rejected') => {
    if (isAdminReadOnly || !pendingAiProposal) return;
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message || data.code || 'Không thể cập nhật góp ý AI');
      await refreshAcademicData();
      addToast({
        type: 'success',
        title: decision === 'revised' ? 'Đã lưu bổ sung của giáo viên' : 'Đã cập nhật góp ý AI',
        message: decision === 'revised' ? 'Học sinh sẽ thấy phần bổ sung của giáo viên cùng góp ý AI.' : 'Đã cập nhật trạng thái góp ý AI.'
      });
    } catch (error: unknown) {
      addToast({ type: 'error', title: 'Không thể cập nhật góp ý AI', message: error instanceof Error ? error.message : 'Vui lòng thử lại.' });
    } finally {
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                ...
                {pendingAiProposal && evaluatorRole === 'teacher' && <Badge size="sm" variant="blue">Có góp ý AI</Badge>}{isAdminReadOnly && <Badge size="sm" variant="outline">Chỉ xem</Badge>}
              </div>
      ...
      <div className="mx-auto grid w-full max-w-[100rem] grid-cols-1 lg:grid-cols-[250px_minmax(0,1fr)_390px]">
        ...
        <main className="min-w-0 space-y-5 p-4 sm:p-6 lg:p-7">
            ...
            <section className="space-y-3 rounded-xl border border-sky-200 bg-sky-50/70 p-4 shadow-xs">
              ...
              {!isAdminReadOnly && (
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  <Button size="sm" variant="ghost" isLoading={isSubmittingAiReview} onClick={() => handleTeacherAiDecision('rejected')}>Không sử dụng</Button>
                </div>
              )}
            </section>
          )}
          ...
          <section className="space-y-4">
            <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-slate-200 pb-3"><div><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Phiên bản đã nộp</div><h2 className="mt-1 text-lg font-bold tracking-tight text-slate-950">{POETIC_AXES.find(axis => axis.id === activeAxisId)?.title}</h2></div><div className="flex items-center gap-2"><Badge variant="outline">{selectedSnapshot.versionNumber}</Badge><span className="text-xs text-slate-400">{activeResponse?.analysisText ? `${activeResponse.analysisText.trim().split(/\s+/).length} từ` : '0 từ'}</span></div></div>
            {selectedText && !isAdminReadOnly && <div className="flex items-center justify-between rounded-lg border border-primary-200 bg-primary-50/80 px-3.5 py-2 text-xs"><div className="truncate text-primary-900"><span className="font-semibold">Đã chọn đoạn:</span> &ldquo;{selectedText.slice(0, 70)}{selectedText.length > 70 ? '...' : ''}&rdquo;</div><div className="flex items-center gap-1.5 shrink-0 ml-2"><Button size="sm" variant="primary" onClick={attachSnippetToComment} leftIcon={<PencilSquareIcon className="h-3.5 w-3.5" />}>Gắn nhận xét</Button><Button size="sm" variant="ghost" onClick={() => setSelectedText('')}>Bỏ chọn</Button></div></div>}
            ...
            {activeResponse?.evidenceQuotes && activeResponse.evidenceQuotes.length > 0 && <div className="space-y-2.5 rounded-xl border border-slate-200 bg-slate-50/60 p-4"><div className="text-xs font-bold uppercase tracking-wider text-slate-500">Dẫn chứng học sinh ({activeResponse.evidenceQuotes.length})</div><div className="space-y-2">{activeResponse.evidenceQuotes.map(item => <blockquote key={item.id} className="rounded-md border-l-3 border-primary-400 bg-white p-3 text-xs italic leading-relaxed text-slate-700 shadow-xs">&ldquo;{item.text}&rdquo;</blockquote>)}</div></div>}
          </section>

          {currentFeedbacks.length > 0 && <section className="space-y-3 border-t border-slate-200 pt-5"><div className="flex items-center justify-between"><h3 className="text-sm font-bold text-slate-800">Nhận xét ở trục này ({currentFeedbacks.length})</h3></div><div className="divide-y divide-slate-100 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xs">{currentFeedbacks.map(item => <div key={item.id} className="space-y-1.5 p-3.5 text-xs"><div className="flex items-center justify-between"><strong className="font-semibold text-slate-800">{item.authorName}</strong><Badge size="sm" variant={item.authorRole === 'teacher' ? 'primary' : 'outline'}>{item.authorRole === 'teacher' ? 'Giáo viên' : 'Học sinh'}</Badge></div>{item.selectedSnippet && <div className="rounded border-l-2 border-slate-300 bg-slate-50 p-2 italic text-slate-600">&ldquo;{item.selectedSnippet}&rdquo;</div>}<p className="leading-relaxed text-slate-800">{item.comment}</p></div>)}</div></section>}
        </main>
