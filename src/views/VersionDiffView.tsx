        ...
        {POETIC_AXES.map(axis => {
          ...
          return <section key={axis.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="grid divide-y divide-slate-200 lg:grid-cols-2 lg:divide-x lg:divide-y-0">
              <div className="p-4"><div className="mb-2 text-xs font-bold text-slate-500">{before.versionNumber}</div><div className="min-h-24 whitespace-pre-wrap font-serif text-sm leading-7 text-slate-700">{beforeText || <span className="font-sans italic text-slate-400">Chưa có nội dung</span>}</div></div>
              <div className={`p-4 ${changed?'bg-indigo-50/30':''}`}><div className="mb-2 text-xs font-bold text-indigo-600">{after.versionNumber}</div><div className="min-h-24 whitespace-pre-wrap font-serif text-sm leading-7 text-slate-800">{afterText || <span className="font-sans italic text-slate-400">Chưa có nội dung</span>}</div></div>
            </div>
